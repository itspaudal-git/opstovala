...
db.ref(path).on(
  ...
  (snapshot) => {
    if (!snapshot.exists()) {
      document.getElementById("tasksContainer").innerHTML = `No data found for term ${term}`;
      allFetchedTasks = [];
      return;
    }
...
const columns = [
  ...
  // Sleeving fields
  { key: "numbersOfTrays", label: "Total # of Trays" },
...
taskEntries.forEach((entry) => {
  ...
  columns.forEach((col) => {
    ...
    // If it's the Start or End column, convert from 24-hr (e.g. "13:14") to 12-hr (00:00 AM/PM)
    if (col.key === "start" || col.key === "end") {
      value = to12HourFormat(value);
    }
    ...
    td.addEventListener("blur", () => {
      const newDisplayValue = td.textContent.trim();
      const oldValue = entry[col.key];

      // Prepare the actual value to store
      let toStoreValue = newDisplayValue;

      // If the column is start/end, convert user’s 12-hr string back to 24-hr
      if (col.key === "start" || col.key === "end") {
        toStoreValue = to24HourFormat(newDisplayValue);
      }
      // Boolean columns
      else if (typeof oldValue === "boolean") {
        // interpret "yes" or "true" or "1" as true, else false
        toStoreValue = /^(yes|true|1)$/i.test(newDisplayValue);
      }

      // Only update if changed (comparing to the original entry's stored value).
      // If oldValue is boolean, oldValue.toString() => "true"/"false"
      // If oldValue is string time (e.g. "13:14"), compare it directly with "13:14".
      // You may need to refine this check if you want to catch format changes differently.
      if (toStoreValue.toString() !== oldValue.toString()) {
...
switch (fieldName) {
  ...
  case "numbersOfTrays":
    return "numbersoftrays"; // JavaScript vs DB
  case "compCheck":
    return "compchecks";
  case "sealsCheck":
    return "sealscheck";
...
if (!time24 || !time24.includes(":")) return time24; // fallback if missing or invalid
...
const hour12 = hour < 10 ? "0" + hour : "" + hour;

return `${hour12}:${minute} ${amPm}`;
}

function to24HourFormat(time12) {
// Expecting something like "00:30 AM" or "01:14 PM", etc.
if (!time12 || !time12.includes(":")) return time12; // fallback if missing or invalid

// Split the time portion from AM/PM
let [timePortion, amPm] = time12.split(" ");
if (!timePortion || !amPm) return time12; // fallback

let [hourStr, minute] = timePortion.split(":");
let hour = parseInt(hourStr, 10);
...
let hour24 = hour < 10 ? "0" + hour : "" + hour;
