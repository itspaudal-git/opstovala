...
function fetchData() {
    ...
    fetch(url)
        ...
        .then(data => processAndDisplayData(data)) // Call the function to process and display the data
        .catch(error => {
...
function processAndDisplayData(packageJson) {
    ...
    cycle1.forEach((item, index) => {
        ...
        if (!name.includes("PAU")) {
            ...
            const sleevingDay = item['tags'].find(tag => tag['category'] === 'sleeving_day')?.title || 'N/A';
        // Store these variables in the row so they can be accessed when generating the PDF
            ...
            if (previousMealNo !== null && mealNo !== previousMealNo) {
                ...
                dividerRow.innerHTML = '<td colspan="12"></td>'; // Divider line
...
<select
    ...
    onchange="updateSleevingConfig('${mealNo}', '${meal_id}', this.value, document.getElementById('bins-configuration-${mealNo}').value, '${sleevingLoc}', document.getElementById('sleeving-day-${mealNo}').value)">
    ...
    `).join('')}
                ...
                <td>
<select
    id="sleeving-day-${mealNo}"
    class="sleeving-day-dropdown"
    onchange="updateSleevingConfig('${mealNo}', '${meal_id}', document.getElementById('sleeving-configuration-${mealNo}').value, document.getElementById('bins-configuration-${mealNo}').value, this.value)">
    ${SleevingdaysOptions.map(option => `
        <option value="${option}" ${option === sleevingDay ? 'selected' : ''}>${option}</option>
    `).join('')}
</select>
...
<select
    ...
    onchange="updateSleevingConfig('${mealNo}', '${meal_id}', document.getElementById('sleeving-configuration-${mealNo}').value, this.value, document.getElementById('sleeving-day-${mealNo}').value)">
    ...
    `).join('')}
            ...
            otherItems.forEach(item => {
                ...
                materialRow.innerHTML = `
                    ...
                    <td>${item.day1}</td>
                    <td>${item.day2}</td>
                    <td>${item.mapTag !== '' ? item.mapTag : ''}</td> <!-- Only display MAP if it's not 'no MAP' -->
                    <td>${item.trayTag !== '' ? item.trayTag : ''}</td> <!-- Only display Tray if it's not 'normal' -->
                    <td>${item.task}</td>
                    <td><button class="download-c1">Download C1</button></td>
                    <td><button class="download-c2">Download C2</button></td>
                `;
...
function updateSleevingConfig(mealNo, meal_id, newConfig, newBinsConfig, newSleevingDay) {
    const term = document.getElementById('searchTerm').value.trim();
    ...
    // If newSleevingDay is not passed, fetch the current value from the dropdown
    if (!newSleevingDay) {
        newSleevingDay = document.getElementById(`sleeving-day-${mealNo}`).value || 'N/A';
    }
    ...
    console.log({
        ...
        newBinsConfig,
        newSleevingDay,
        sleevingLoc  // Include sleeving location for the payload
    });

    // Provide fallback values if necessary, e.g., if any of these are empty or undefined, use 'N/A'
    newConfig = newConfig || 'N/A';
    newBinsConfig = newBinsConfig || 'N/A';
    newSleevingDay = newSleevingDay || 'N/A';

    // Update the target URL to include '/tags'
    const url = `https://misevala-api.tvla.co/v0/productionRecords/${term}/productionMeals/${meal_id}/tags`;

    // Build the payload, including 'sleeving_location'
    const payloadToSend = {
        "tags": [
            { "category": "sleeving_location", "title": sleevingLoc },  // Ensure sleeving_location is included
            { "category": "sleeving_configuration", "title": newConfig },  // New sleeving configuration from dropdown
            { "category": "sleeving_bins_configuration", "title": newBinsConfig }, // New bins config from the second dropdown
            { "category": "sleeving_day", "title": newSleevingDay }  // New sleeving day from dropdown
        ]
    };
    ...
    fetch(url, {
        ...
        body: JSON.stringify(payloadToSend)  // Wrap the tags array inside a 'tags' key
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(text => {
        if (text) {
            ...
            document.getElementById('status').textContent = `Configuration for Meal ${mealNo} updated successfully.`;
        } else {
            console.log('No response body received.');
            document.getElementById('status').textContent = `Configuration for Meal ${mealNo} updated successfully with no response data.`;
        }
    })
    .catch(error => {
        console.error('Error updating:', error);
        document.getElementById('status').textContent = `Error updating configuration for Meal ${mealNo}: ${error.message}`;
    });
}
